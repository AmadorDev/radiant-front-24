...
function salons({ data, translation }) {
      ...
      <div className="container flex flex-col lg:flex-row justify-between space-y-5 lg:space-y-0 lg:space-x-5">
        <div className="w-full lg:w-[550px]">
          ...
          <div className="map-h-content px-2 mt-2 map-scroll">
            {data.map((item, index) => (
              <div className={`map-card-item p-3 mb-3 cursor-pointer ${active === item.id ? 'active' : ''}`}
                key={index}
        ...
        <div className="w-full h-[550px]">
          <MapFinal center={center} zoom={zoom} text={storeName} />
        </div>
      </div>
